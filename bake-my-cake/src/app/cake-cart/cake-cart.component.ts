...
customerflatNo: any;
  ...
  canDeactivate() {
    if (!this.submitStatus)
      this.submitStatus = confirm(
        'You have not submitted a request to this bakery items. Any details entered will be lost. Are you sure you want to leave?'
      );
  ...
  TotalPrice() {
    if (
      this.cake &&
      typeof this.cake.price === 'number' &&
      this.cakeRequest.quantity
    ) {
      if (this.cakeRequest?.quantity > 0) {
        this.cakeRequest.totalAmount =
          this.cake.price * this.cakeRequest.quantity;
      }
    } else {
